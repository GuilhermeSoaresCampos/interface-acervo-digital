    ...
    return (
        <main className="bg-gray-200 flex-1 flex flex-col px-4 sm:px-6 md:px-10 py-6 md:py-10 overflow-hidden"> {/* overflow-hidden no main para conter o scroll interno */}
            <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row items-center gap-4 mb-6 md:mb-8 flex-shrink-0">
                <h1 className="flex-1 text-xl sm:text-2xl md:text-3xl text-center sm:text-left font-bold text-slate-800">Emprestimos</h1>
            ...
            <input type="text" name="busca-emprestimo" id="busca-emprestimo" placeholder="Buscar emprestimo" className="w-full max-w-6xl mx-auto p-3 md:p-2 md:mb-4 border-b-2 border-slate-700 rounded-sm" />
                    ...
                    <table className="table-auto w-full border-collapse text-xs sm:text-sm md:text-base">
                        <thead className="bg-slate-700 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th className="border-b border-slate-600 text-white p-3 md:p-4 hidden sm:table-cell text-left">Aluno</th>
                                <th className="border-b border-slate-600 text-white p-3 md:p-4 hidden md:table-cell text-left">Data</th>
                                <th className="border-b border-slate-600 text-white p-3 md:p-4 hidden lg:table-cell text-left">Status</th>
                            ...
                            {currentEmprestimos && currentEmprestimos.length > 0 ? (
                                currentEmprestimos.map((emprestimo) => (
                                    <tr className="text-center md:text-left transition-colors hover:bg-slate-50 group" key={emprestimo.id_emprestimo}>
                                        ...
                                        <td className="p-3 md:p-4 font-medium text-slate-700">{emprestimo.livro?.titulo ?? "Sem livro"}</td>
                                        <td className="p-3 md:p-4 hidden sm:table-cell text-slate-600">{`${emprestimo.aluno?.nome ?? ""} ${emprestimo.aluno?.sobrenome ?? ""}`.trim() || "Sem aluno"}</td>
                                        <td className="p-3 md:p-4 hidden md:table-cell text-slate-600">{emprestimo.data_emprestimo ? new Date(emprestimo.data_emprestimo).toLocaleDateString('pt-BR') : "--"}</td>
                                        <td className="p-3 md:p-4 hidden lg:table-cell text-slate-600">{emprestimo.status_emprestimo ?? "--"}</td>
